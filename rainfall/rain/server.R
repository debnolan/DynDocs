...
shinyServer(function(input, output) {
  ...
  output$plots5 <- renderPlot({
    plot_map(datasetInput2())
  })
  output$plots3 <- renderPlot({
    up = subset(rainfall[[datasetInput()]], precip > quantile(rainfall[[datasetInput()]]$precip, input$threshold/100))
    #temp = mdply(rainfall[[datasetInput()]], function(precip,...) {ifelse(precip > quantile(rainfall[[datasetInput()]]$precip, input$threshold/100), 'darkblue', 'lightblue')})
    plot(rainfall[[datasetInput()]]$precip ~ rainfall[[datasetInput()]]$time, xlab = "", ylab = "Precipitation (mm)", main = paste("Daily precipitation of weather station", datasetInput()), col = 'lightblue')
    points(x = up$time, y = up$precip, col = 'darkblue')
    abline(h = quantile(rainfall[[datasetInput()]]$precip, input$threshold/100), col = 'red')
  })
  ...
  sim <- eventReactive(input$goButton, {
    fit = fevd(precip, rainfall[[datasetInput2()]], threshold = quantile(rainfall[[datasetInput2()]]$precip, input$threshold2/100), type="GP", units = "mm")
    plot(fit, "qq2")
  ...
  output$rl2 <- renderText({
    fit = fevd(precip, rainfall[[datasetInput2()]], threshold = quantile(rainfall[[datasetInput2()]]$precip, input$threshold2/100), type="GP", units = "mm")
    RV2 = ci(fit, return.period=c(2,5,10,20,50,80,100,120,200))
    myprint(RV2)
  })
