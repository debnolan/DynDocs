...
shinyServer(function(input, output) {
  ...
  ###################### sidebyside_2
   ...
   output$info_1 <- renderPrint({
     nearPoints(data(), input$plot_hover_1, xvar = input$x_1, yvar = input$y_1, threshold = 7)
   })
   
   output$info_2 <- renderPrint({
     nearPoints(data(), input$plot_hover_2, xvar = input$x_2, yvar = input$y_2, threshold = 7)
   })
